...
| 6 | **DeepSeek becomes the default generator, not yet switched.** Decided 18 Sep (`docs/DECISIONS_LLM_ROLES.md` D1); `configs/verification.yaml` still says `template`. Order: W1 column check, W2 pin the tests to the template, then W3 flip the default; a deployed setup also needs W13 (generation outside the offline worker). | Switching the default before W1 would let LLM-written code test the wrong columns with a valid stamp. |
| 7 | **No column check on generated code (W1), no template-agreement check (W8).** The HMAC stamp proves numbers came from our library, not that the code used the claim's own columns and rows. | W1 blocks row 6. W8 is also the DeepSeek-vs-template comparison for the evaluation. |
